...
public class PrepareAnvil implements Listener {
    ...
    public void onPrepare(PrepareAnvilEvent e) {
        ...
        if (!BackPacks.isBackpack(e.getResult())) return;

        ItemMeta im = e.getResult().getItemMeta();

        Objects.requireNonNull(im).setDisplayName(Chat.colorize(Objects.requireNonNull(Files.getInstance().getconfig().getString("backpack.name.renamed")).replace("%name%", Objects.requireNonNull(e.getInventory().getRenameText()))));
